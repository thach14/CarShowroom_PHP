...
if(isset($_SESSION['activestaff']) || isset($_SESSION['activeadmin'])){
    ...
    if(isset($_GET["search"]) && $_GET["input"]!='') {
      ...
      $sql_forms = $db->query("select t.id, c.make, c.name , c.model, c.year,t.date,t.time,u.email
                               from car c,customer u,testdrive t
                               WHERE 1 
                               and c.id = t.car
                               and u.id = t.customer".$condition);

      $pages->items_total = $sql_forms->rowCount();
      $pages->mid_range = 9;
      $pages->paginate();  
      
      $result =   $db->query("select t.id, c.make, c.name , c.model, c.year,t.date,t.time,u.email
                              from car c,customer u,testdrive t
                              WHERE 1 
                              and c.id = t.car
                              and u.id = t.customer".$condition."
                              order by t.date desc ".$pages->limit."");
    ...
    $pages = new Paginator;        
    $pages->default_ipp = 10;
    $sql_forms = $db->query("SELECT * FROM testdrive WHERE 1");
    $pages->items_total = $sql_forms->rowCount();
    $pages->mid_range = 9;
    $pages->paginate();  
          ...
          <div class="collapse navbar-collapse" id="navbarCollapse">
            ...
            <ul class="navbar-nav mx-auto">
              ...
              <li class="nav-item position-static">
                <!-- Toggle -->
                <a class="nav-link active" href="testSchedule.php?page=1&ipp=2">Test Drive</a>
              </li>
              <li class="nav-item position-static">
                <!-- Toggle -->
      ...
      <div class="col-lg-10 col-xl-9 mx-auto">
        <div class="card card-signin flex-row my-5">
          <div class="card-body">
            ...
            <div class="d-flex">
            <form method="get" action="testSchedule.php" class="form-inline">
                ...
                <button type="button" class="btn btn-outline-primary ml-2" onclick="window.document.location.href='testSchedule.php?page=1&ipp=2'">Reset</button>  
            </form>
            <form action="testSchedule.php" method="get">
                <input type="hidden" name="ipp" value='2'>
  	            <input type="hidden" name="page" value='1'>        
...
<script>  
  function print(){
      ...
      mywindow.document.write('<html><head><title>Cars Sales</title></br><h3 align=\'center\'>Test Drive Schedule - <?php echo $_GET['year'].'/'.$_GET['month']?></h3><hr>');
      <?php } else{?>
      mywindow.document.write('<html><head><title>Cars Sales</title></br><h3 align=\'center\'>Test Drive Schedule</h3><hr>');
      <?php }?>
      mywindow.document.write('</head><body><table style="width:100%"><tr><th>ID</th><th>Car</th><th>Customer Account</th><th>Date-Time</th></tr>');
      
      for (let index = 0; index < car.length; index++) {
          mywindow.document.write('<tr>');
          mywindow.document.write('<td align=\'center\'>'+car[index][0]+'</td>');
          mywindow.document.write('<td align=\'center\'>'+car[index][1]+' '+car[index][2]+' '+car[index][3]+' '+car[index][4]+'</td>');
          mywindow.document.write('<td align=\'center\'>'+car[index][7]+'</td>');
          mywindow.document.write('<td align=\'center\'>'+car[index][5]+'  '+car[index][6]+':00</td>');
          mywindow.document.write('</tr>');
      }                      
